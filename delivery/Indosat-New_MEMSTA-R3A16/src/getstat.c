...
//Adapted to read data for diskstat independent of time format
//#define DEBUG

void getstat_read_data_disk(getstat_data_t info_type, FILE *p, char *tokens[], int row_terminator, char *data_terminator, int header_fields[]) {
	int terminator, field, end_of_data = 0;
	char *parsed_string = NULL;
	char *info_values[GETSTAT_MAX_VALUES + 1];
...
// Lines added to ignore "Average:" at the start of each data row		
				{
					char *parse_line = 0;
