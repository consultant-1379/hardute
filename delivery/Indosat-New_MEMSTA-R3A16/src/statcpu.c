...
//	char *header_names_new[] = CPUCORESTATALT_NAMES;
...
//	p1 = popen(CPUCORESTAT_COMMAND, "r");
...
/*#if defined (CPUCORESTAT_USERTOKEN)
	{
	char *parse_header = 0;
	printf(">>%s\n", CPUCORESTAT_USERTOKEN);
	parse_header = read_file_to_string_arg(p, CPUCORESTAT_USERTOKEN, 0);
	printf("parse_header = \"%s\"\n", parse_header);
	if (parse_header) {
		free(parse_header);
		parse_header = 0;
		getstat_read_header(p, header_tokens, '\n', header_names_new, 0, header_pattern);
		}
		else {
		free(parse_header);
		parse_header = 0;
